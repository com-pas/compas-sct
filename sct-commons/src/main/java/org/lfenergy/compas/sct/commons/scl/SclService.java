...
public class SclService {
            ...
            for (SubNetworkDTO subNetworkDTO : subNetworks) {
                ...
                for (ConnectedApDTO accessPoint : subNetworkDTO.getConnectedAPs()) {
                    ...
                    if (subNetworkAdapter.isPresent()) {
                        ...
                        copyAddressAndPhysConnFromIcd(apName, icd, connectedAPAdapter);
                    }
    ...
    private static void copyAddressAndPhysConnFromIcd(String apName, Optional<SCL> icd, ConnectedAPAdapter connectedAPAdapterScd) {
        if (icd.isPresent() && icd.get().getCommunication() != null) {
            icd.stream()
                    .map(SCL::getCommunication)
                    .findFirst()
                    .ifPresent(com -> com.getSubNetwork().stream()
                            .flatMap(subNet -> subNet.getConnectedAP().stream()
                                    .filter(connAP -> connAP.getApName().equals(apName)))
                            .findFirst()
                            .ifPresent(connectedAP -> {
                                connectedAPAdapterScd.getCurrentElem().setAddress(connectedAP.getAddress());
                                connectedAPAdapterScd.getCurrentElem().getPhysConn().addAll(connectedAP.getPhysConn());
                            }));
