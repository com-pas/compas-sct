 ...
 */
public record ControlBlockTarget(String apRef, String iedName, String ldInst, LnKey lnKey, String desc) {
     ...
     */
    public ControlBlockTarget(String apRef, String iedName, String ldInst, LnKey lnKey) {
        this(apRef, iedName, ldInst, lnKey, null);
    }
    ...
    public TClientLN toTClientLn() {
        ...
        if (StringUtils.isNotBlank(lnKey.lnClass())) {
            newTClientLn.getLnClass().add(lnKey.lnClass());
        }
        // LnInst is required on TClientLN, so it cannot be null, but it can be empty
        newTClientLn.setLnInst(StringUtils.trimToEmpty(lnKey.inst()));
        newTClientLn.setPrefix(StringUtils.trimToNull(lnKey.prefix()));
    ...
    public IEDName toIedName() {
        ...
        if (StringUtils.isNotBlank(lnKey.lnClass())) {
            newIedName.getLnClass().add(lnKey.lnClass());
        }
        // LnInst is optional on IEDName, so it can be null, but cannot be empty
        newIedName.setLnInst(StringUtils.trimToNull(lnKey.inst()));
        newIedName.setPrefix(StringUtils.trimToNull(lnKey.prefix()));
        return newIedName;
    }
    ...
    public boolean equalsIedName(IEDName iedName) {
        return Objects.equals(iedName.getApRef(), apRef)
                ...
                && Objects.equals(LnKey.from(iedName), lnKey);
    }
    ...
    public boolean equalsTClientLn(TClientLN tClientLn) {
        return Objects.equals(tClientLn.getApRef(), apRef)
                ...
                && Objects.equals(LnKey.from(tClientLn), lnKey)
